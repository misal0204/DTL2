...
public class DocumentsBean implements Serializable {
    ...
    public void toFecha(Date d) {
        ...
        try {
            ...
            TypedQuery<DlEncamh> query
                    = em.createNamedQuery("DlDetmh.findByFecha", DlEncamh.class).setParameter("fecha",tofecha);
            List<DlEncamh> results = query.getResultList();

            documentos = results;

            for (DlEncamh document : results) {
                System.out.println("Documento según fecha: " + document.getDocumento());
            }
        } catch (PersistenceException pe) { 
            System.out.println("Mensaje error en fecha1 bean: " + pe.getMessage());
            System.err.println("Error DBConnection: " + pe.getMessage());
            em.close();
            emf.close();
        }
